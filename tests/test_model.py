...
def test_discriminant_db_shape(dummy_input):
    ...
    p = model.predict_proba(**dummy_input)
    d = model.discriminant_db(p,
                              flavour_map={"b-jet": 0, "c-jet": 1, "light-jet": 2, "tau-jet": 3})
...
def test_discriminant_db_invalid_label_map(dummy_input):
    ...
    with pytest.raises(ValueError):
        model.discriminant_db(p, flavour_map=bad_map)
...
def test_discriminant_dc_invalid_label_map(dummy_input):
    ...
    with pytest.raises(ValueError):
        model.discriminant_dc(p, flavour_map=bad_map)